...
include_once(INCLUDE_DIR.'class.equipment_status.php');
...
class Ticket {
    ...
    function close() {
        ...
        Equipment::onCloseTicket($this->getId());
        $this->reload();
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        if(!db_query($sql) || !db_affected_rows())
          ...
          if($this->updateEquipment($vars, true))
          {
              $errors['equipment']='Unable to update equipment!';
          }
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    /**********************************************************************/
    /********************  Equipment Update *******************************/
    function updateEquipment($vars, $is_update=false)
    {
        if(isset($vars['equipment_id']) && isset($vars['status_id']))
        {
    ...
    function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
        ...
        if(!db_query($sql) || !($id=db_insert_id()) || !($ticket =Ticket::lookup($id)))
          ...
          if($ticket->updateEquipment($vars, false))
          {
              $errors['equipment']='Unable to update equipment1';
          }
