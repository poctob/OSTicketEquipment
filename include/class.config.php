...
class Config {
    ...
    function isKnowledgebaseEnabled() {
     ...
     function isEquipmentEnabled() {
        ...
        return ($this->config['enable_equipment'] && Equipment::countPublishedEquipment());
    }
        ...
        switch(strtolower($vars['t'])) {
           ...
           case 'autoresp':
            ...
            case 'kb':
                ...
                break;
            case 'equipment':
                return $this->updateEquipmentSettings($vars, $errors);
                break;
            default:
    ...
    function updateEquipmentSettings($vars, &$errors) {

        if($errors) return false;

        $sql = 'UPDATE '.CONFIG_TABLE.' SET updated=NOW() '
              .',enable_equipment='.db_input(isset($vars['enable_equipment'])?1:0)
              .' WHERE id='.db_input($this->getId());
